...
status_file='/var/www/localhost/htdocs/status'
date="$(date +"%Y-%m-%d %H:%M:%S")"
if [ -z $IONOS_UPDATE_URL ]; then
  echo "ERROR: Variables no configurada IONOS_UPDATE_URL"
  echo "{\"\date\":\"${date}\",\"error\":\"Missing IONOS_UPDATE_URL environment variable\"}" > $status_file
  exit 1
fi
...
echo $parsed_json | jq ". + {date: \"${date}\", return_code: \"${code}\"}" | tee $status_file
rm $temp_file
